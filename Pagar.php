...
if ($_SERVER['REQUEST_METHOD'] === "POST") {
    ...
    if ($result->num_rows == 1) {
        ...
        if ($validar) {
            echo '<scrip>
    alert ("cinora regustrada");
    window.location("todFacturas.php? id=' . $idp . '","_black");
    window.location.href="todFacturas.php?id=' . $idp . '";
    </scrip>';
        } else {
            echo '<scrip alert("error de compra");
    window.location.href="index.php";
        ...
        if ($validar) {
            ...
            window.location.href='factura.php' </script>";
